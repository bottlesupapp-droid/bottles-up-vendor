-- =====================================================
-- ACTUAL FIX MIGRATION (Based on Real Schema)
-- Date: April 3, 2026
-- Purpose: Fix Analytics and Venues errors with correct table names
-- =====================================================

-- Step 1: Check which booking table to use
-- First, let's see which columns exist in events_bookings
DO $$
BEGIN
...
-- Step 3: Enhance events_bookings table for QR scanning
DO $$
BEGIN
  -- Add ticket_code
...
-- Step 4: Create new feature tables
...
-- Step 5: Create analytics views using CORRECT table name (events_bookings)
-- IMPORTANT: Adjust column names based on your actual schema

-- First, let's check what columns exist in events and events_bookings
-- You may need to adjust the column names in the views below
...
CREATE POLICY "Venue owners can view their boosts"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM venues WHERE venues.id = venue_boosts.venue_id AND venues.user_id = auth.uid()
    )
  );

DROP POLICY IF EXISTS "Venue owners can manage boosts" ON venue_boosts;
CREATE POLICY "Venue owners can manage boosts"
